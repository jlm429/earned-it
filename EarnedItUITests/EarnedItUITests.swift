...
final class EarnedItUITests: XCTestCase {
    ...
    func testAlternatingChoreShowsOnlyTheCurrentChildTurn() throws {
        ...
        XCTAssertEqual(app.staticTexts["alternating-turn-order"].label,
                       "Turn order: Nora, Alek, Hanna. It advances with each scheduled date.")
        ...
        XCTAssertTrue(app.staticTexts["full-status-set-the-table"].label.contains("Nora’s turn"))
        XCTAssertTrue(app.buttons["state-set-the-table-nora"].exists)
    ...
    func testAlternatingAsNeededShowsNextSkipOwnerAndImmediateDelete() {
        ...
        tap("as-needed-chores")
        tap("add-as-needed-chore")
        fill("responsibility-title", with: "Empty dishwasher")
        tap("chore-requirement")
        tap("Alternate / take turns")
        app.switches["eligible-hanna"].switches.firstMatch.tap()
        app.switches["eligible-alek"].switches.firstMatch.tap()
        ...
        tap("alternating-first-child")
        tap("Nora")
        XCTAssertTrue(app.staticTexts["alternating-turn-order"].label.contains("Completing it advances the turn"))
        tap("save-responsibility")

        XCTAssertEqual(app.staticTexts["next-turn-empty-dishwasher"].label, "Whose Turn: Nora")
        tap("skip-next-empty-dishwasher")
        XCTAssertEqual(app.staticTexts["next-turn-empty-dishwasher"].label, "Whose Turn: Alek")
        tap("make-available-empty-dishwasher")
        XCTAssertTrue(app.staticTexts["Available Today for Alek"].waitForExistence(timeout: 5))
        keepScreenshot("as-needed-alternating-active-owner")

        app.navigationBars.buttons.element(boundBy: 0).tap()
        app.navigationBars.buttons.element(boundBy: 0).tap()
        XCTAssertTrue(app.buttons["state-empty-dishwasher-alek"].waitForExistence(timeout: 5))
        XCTAssertFalse(app.buttons["state-empty-dishwasher-hanna"].exists)
        tap("switch-user")
        tap("user-card-alek")
        XCTAssertTrue(app.buttons["state-empty-dishwasher-alek"].waitForExistence(timeout: 5))
        XCTAssertFalse(app.buttons["state-empty-dishwasher-hanna"].exists)
        ...
        XCTAssertTrue(app.alerts["Delete \"Empty dishwasher\"?"].waitForExistence(timeout: 5))
        app.alerts.buttons["Delete"].tap()
        XCTAssertFalse(app.staticTexts["Empty dishwasher"].waitForExistence(timeout: 2))
        app.navigationBars.buttons.element(boundBy: 0).tap()
        app.navigationBars.buttons.element(boundBy: 0).tap()
        XCTAssertFalse(app.staticTexts["Empty dishwasher"].exists)
        tap("switch-user")
        tap("user-card-alek")
        XCTAssertFalse(app.staticTexts["Empty dishwasher"].exists)
        keepScreenshot("deleted-chore-removed-for-child")
    }
