...
final class EarnedItUITests: XCTestCase {
    ...
    func testAlternatingChoreShowsOnlyTheCurrentChildTurn() throws {
        ...
        XCTAssertEqual(app.staticTexts["alternating-turn-order"].label,
                       "Turn order: Alek, Hanna, Nora. It advances with each scheduled date.")
    ...
    func testAlternatingAsNeededShowsNextSkipOwnerAndImmediateDelete() {
        ...
        XCTAssertEqual(app.staticTexts["next-turn-empty-dishwasher"].label, "Whose Turn: Hanna")
        tap("make-available-empty-dishwasher")
        XCTAssertTrue(app.staticTexts["Available Today for Hanna"].waitForExistence(timeout: 5))
        keepScreenshot("as-needed-alternating-active-owner")

        app.navigationBars.buttons.element(boundBy: 0).tap()
        app.navigationBars.buttons.element(boundBy: 0).tap()
        XCTAssertTrue(app.buttons["state-empty-dishwasher-hanna"].waitForExistence(timeout: 5))
        XCTAssertFalse(app.buttons["state-empty-dishwasher-alek"].exists)
        tap("switch-user")
        tap("user-card-hanna")
        XCTAssertTrue(app.buttons["state-empty-dishwasher-hanna"].waitForExistence(timeout: 5))
        XCTAssertFalse(app.buttons["state-empty-dishwasher-alek"].exists)
