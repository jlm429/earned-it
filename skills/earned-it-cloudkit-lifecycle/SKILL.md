...
- Apply that exact participant check again after awaited claim and lock work, immediately before cleanup, retry, resume, or startup recovery synchronously attaches its profile. Bind claim submission to the authenticated participant and account generation. When pending state names an invitation, the committed membership must name that same invitation.
- Pending invitation acceptance owns onboarding until attachment or cleanup finishes. Block family creation and revalidate the unchanged empty or exact continuing-import session immediately before cleanup or recovery attaches or clears local state.
