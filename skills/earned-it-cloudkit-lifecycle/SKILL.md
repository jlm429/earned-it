...
- Capture exact share-access state before raw native acceptance. Reject preexisting accepted access or an active membership lock, and promote a retryable acceptance error only when the persisted attempt began without access and the new exact participant uniquely matches one invitation. Retain retryable pending state when the result is uncertain.
