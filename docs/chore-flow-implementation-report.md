...
F1 remains open: an existing any-one/multiple chore edit can retain an archived child's hidden ID and reject saving. It does not leak archived children into all-mode projections, whose revisions have no explicit member IDs; centralized validation rejects ineligible explicit assignments. It is not required to change that editor policy for the accepted all-children behavior. The queued legacy timezone migration and same-day particular reassignment follow-ups are not expanded. Current civil dates, next-day edits/archives, and recorded history retain their coverage.
