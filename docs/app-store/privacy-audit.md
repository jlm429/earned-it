...
| Local data | SwiftData stores family facts and a device session in Application Support. Debug UI tests use a separate Documents store only under `DEBUG`. |
| iCloud data | Family facts use the owner's private CloudKit zone and invited participants' shared database. Private account-membership records bind the iCloud participant to one household membership. |
...
| Logging | No `print`, `NSLog`, `os_log`, or `Logger` calls were found in application source. |
| Required-reason APIs | The app creates its Application Support directory with `FileManager`, but does not use the covered file-timestamp, system-boot-time, disk-space, or user-defaults APIs identified in source. |

## Privacy manifest

`EarnedIt/PrivacyInfo.xcprivacy` declares no tracking domains and no required-reason API access. It declares the data types that the app retains off device through CloudKit for app functionality:
