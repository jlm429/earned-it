...
final class HouseholdStore {
    ...
    private enum MissingFamilyAccessOutcome: Equatable {
        case notMissing
        case familyDeleted
        case accessLost
    }
    ...
    var latestInvitationDiagnostics: String? { transport?.familyTransitionDiagnostics.latestInvitationTrace }

    func recordInvitationErrorBeforePresentation(_ error: Error) {
        transport?.familyTransitionDiagnostics.recordUserFacingErrorConversion(error)
    }
    ...
    private func withExclusiveCloudMutation<T>(
        ...
        let shouldRestoreScheduledSync = cancelScheduledSync && syncTask != nil && pendingCount > 0
    ...
    func prepareFamilyAccessLostUITest() throws {
    ...
    func prepareInvitationActionsUITest(isOwner: Bool) throws {
        if household == nil {
            ...
            updated.cloudParticipantID = isOwner ? "synthetic-owner" : "synthetic-invited-parent"
            try repository.commit(facts: [], session: updated)
            session = updated
        }
    }
    ...
    func createFamily(name: String, parentName: String, timeZone: TimeZone = .current) throws {
        ...
        guard session.pendingInvitationAcceptance == nil else {
    ...
    func canRecoverInvitation(_ invitation: FamilyInvitation) -> Bool {
        session.location?.isOwner == true
            && session.location?.householdID == invitation.householdID
            ...
            && InvitationCode.isSHA256Digest(invitation.cloudShareURLDigest)
    }
    ...
    func recoverInvitation(_ invitation: FamilyInvitation) async throws -> RecoveredFamilyInvitation {
        try await withExclusiveCloudMutation(cancelScheduledSync: true) {
            ...
            guard snapshot.invitation(invitation.id) == invitation,
                  ...
                  InvitationCode.isSHA256Digest(expectedURLDigest),
                  !invitation.cloudShareParticipantID.isEmpty,
        ...
        do {
            ...
            if session.location?.isOwner == true {
                ...
                do {
                    ...
                    diagnostics.record(stage: .invitationAccessPruning, outcome: .succeeded,
                                       householdID: household.id)
                } catch {
                    diagnostics.record(stage: .invitationAccessPruning, outcome: .failed,
                                       householdID: household.id, error: error)
                    throw error
                }
            } else {
                diagnostics.record(stage: .invitationAccessPruning, outcome: .skipped,
                                   householdID: household.id)
            }
            diagnostics.record(stage: .invitationGeneration, outcome: .started,
                               householdID: household.id)
            let code: String
            do {
                code = try InvitationCode.generate()
                diagnostics.record(stage: .invitationGeneration, outcome: .succeeded,
                                   householdID: household.id)
            } catch {
                diagnostics.record(stage: .invitationGeneration, outcome: .failed,
                                   householdID: household.id, error: error)
                throw error
            }
            if session.location == nil {
                diagnostics.record(stage: .zoneConnectionBootstrap, outcome: .started,
                                   householdID: household.id)
                do {
                    try await connect()
                    diagnostics.record(stage: .zoneConnectionBootstrap, outcome: .succeeded,
                                       householdID: household.id)
                } catch {
                    diagnostics.record(stage: .zoneConnectionBootstrap, outcome: .failed,
                                       householdID: household.id, error: error)
                    throw error
                }
            } else {
                diagnostics.record(stage: .zoneConnectionBootstrap, outcome: .skipped,
                                   householdID: household.id)
                diagnostics.record(stage: .initialFactSynchronization, outcome: .skipped,
                                   householdID: household.id)
            }
            diagnostics.record(stage: .preInvitationSynchronization, outcome: .started,
                               householdID: household.id)
            do {
                try await synchronize()
                diagnostics.record(stage: .preInvitationSynchronization, outcome: .succeeded,
                                   householdID: household.id)
            } catch {
                diagnostics.record(stage: .preInvitationSynchronization, outcome: .failed,
                                   householdID: household.id, error: error)
                throw error
            }
            diagnostics.record(stage: .invitationStateValidation, outcome: .started,
                               householdID: household.id)
            guard let location = session.location, let parent = selectedMember else {
                diagnostics.record(stage: .invitationStateValidation, outcome: .failed,
                                   householdID: household.id, error: HouseholdError.cloudUnavailable)
                throw HouseholdError.cloudUnavailable
            }
            diagnostics.record(stage: .invitationStateValidation, outcome: .succeeded,
                               householdID: household.id)
            ...
            diagnostics.recordInternalError(error)
    ...
    private func collectInvitationDiagnosticPreamble(household: Household) async throws {
        ...
        diagnostics.record(stage: .invitationDiagnosticPreamble, outcome: .started,
                           householdID: household.id)

        diagnostics.record(stage: .invitationDiagnosticIdentityRead, outcome: .started,
                           householdID: household.id)
        let participant: String?
        do {
            let currentParticipant = try await transport.participantID()
            ...
            diagnostics.record(stage: .invitationDiagnosticIdentityRead, outcome: .succeeded,
                               householdID: household.id, participantID: currentParticipant,
                               accountGenerationStable: transport.accountGeneration == generation)
        } catch {
            participant = nil
            diagnostics.record(stage: .invitationDiagnosticIdentityRead, outcome: .failed,
                               householdID: household.id,
                               accountGenerationStable: transport.accountGeneration == generation,
                               error: error)
        }

        let expectedOwnerBinding = AccountMembershipBinding.owner(householdID: household.id)
        let expectedOwnerAuthorityBinding = participant.map {
            AccountMembershipBinding.ownerAuthority(participantID: $0)
        }
        diagnostics.recordExpectedOwnerMembership(
            householdID: household.id,
            memberID: selectedMember?.id,
            role: selectedMember?.role,
            locationIsOwner: session.location?.isOwner == true,
            participantID: participant
        )
        diagnostics.record(stage: .invitationDiagnosticMembershipLockRead, outcome: .started,
                           householdID: household.id)
        do {
            let lock = try await transport.accountMembershipLock()
            diagnostics.record(stage: .invitationDiagnosticMembershipLockRead,
                               outcome: lock == nil ? .absent : .succeeded,
                               lock: lock, householdID: household.id,
                               accountGenerationStable: transport.accountGeneration == generation)
            ...
            if let location = session.location, let participant {
                let comparison = ownerMembershipComparison(
                    lock: lock,
                    binding: expectedOwnerBinding,
                    imported: snapshot,
                    location: location,
                    participant: participant
                )
                diagnostics.recordOwnerComparison(comparison)
                diagnostics.recordOwnerBranch(
                    comparison.permitsActiveOwnerReuse
                        ? .readOnlyPreambleExactActiveOwnerCandidate
                        : .readOnlyPreambleRequiresOwnerReconciliation,
                    result: "observed"
                )
            } else {
            ...
            diagnostics.record(stage: .invitationDiagnosticMembershipLockRead, outcome: .failed,
                               householdID: household.id,
                               accountGenerationStable: transport.accountGeneration == generation,
                               error: error)
            diagnostics.recordOwnerBranch(.readOnlyPreambleUnavailable, result: "observed")
    ...
    func connect() async throws {
        try await withExclusiveCloudMutation {
        guard !familyAccessLost else { throw HouseholdError.ownerMembershipUnavailable }
        try requireParent()
        ...
        if connectingSession.accountMembershipLockAttemptID != nil {
            let isAuthoritative = try await incompleteOwnerConnectionIsAuthoritative(
                expectedSession: connectingSession,
                householdID: household.id,
                participant: participant,
        ...
        guard session.deviceID == connectingSession.deviceID,
            ...
            transport.familyTransitionDiagnostics.record(stage: .connectionStateValidation, outcome: .failed,
                                                         householdID: household.id,
                                                         error: HouseholdError.noHousehold)
            throw HouseholdError.noHousehold
        }
        transport.familyTransitionDiagnostics.record(stage: .connectionStateValidation, outcome: .succeeded,
                                                     householdID: household.id)
        ...
        transport.familyTransitionDiagnostics.record(stage: .lifecycleAuthorityValidation, outcome: .started,
                                                     householdID: household.id)
        guard lifecycleState == .active else {
            transport.familyTransitionDiagnostics.record(stage: .lifecycleAuthorityValidation, outcome: .failed,
                                                         householdID: household.id,
                                                         error: HouseholdError.accountMembershipConflict)
            throw HouseholdError.accountMembershipConflict
        }
        transport.familyTransitionDiagnostics.record(stage: .lifecycleAuthorityValidation, outcome: .succeeded,
                                                     householdID: household.id)
        transport.familyTransitionDiagnostics.record(stage: .connectionStateValidation, outcome: .started,
                                                     householdID: household.id)
        guard session.deviceID == connectingSession.deviceID,
        ...
        do {
            try await synchronize()
            transport.familyTransitionDiagnostics.record(stage: .initialFactSynchronization, outcome: .succeeded,
                                                         householdID: household.id)
        } catch {
            transport.familyTransitionDiagnostics.record(stage: .initialFactSynchronization, outcome: .failed,
                                                         householdID: household.id, error: error)
            throw error
        }
    ...
    func join(url: URL) async throws {
        ...
        if binding == nil, try await transport.hasAcceptedAccess(to: location) {
            ...
            binding = try await membershipEvidence(
                in: HouseholdSnapshot(facts: remote),
                location: location,
                participant: participant
            ).binding
        }
    ...
    private func redeemInvitation(
        ...
        let existingEvidence = try await membershipEvidence(
            in: imported,
            location: location,
            participant: participant
        )
        if let membership = existingEvidence.membership {
            recordJoinReceipt {
                $0.claim = .committed
            ...
            try await attachAfterValidatingInvitationAccess(
                remote: remote, location: location, participant: participant,
        ...
        do {
            confirmedClaims = try await transport.claimInvitation(
                [invitationFact, accountFact],
                in: location,
                expectedParticipantID: participant,
                expectedInvitationParticipantID: invitation.cloudShareParticipantID,
            ...
            let refreshedEvidence = try await membershipEvidence(
                in: refreshedSnapshot,
                location: location,
                participant: participant
            )
            if let membership = refreshedEvidence.membership,
               membership.claim.codeDigest == invitation.codeDigest,
               membership.invitation.id == invitation.id {
                let canWrite = try await transport.canWrite(to: location)
                let lock = try await activateAccountMembershipLock(
                    location: location, claimBinding: AccountMembershipBinding.invitation(membership),
                    attemptID: attemptID
                ...
                try await attachAfterValidatingInvitationAccess(
                    remote: refreshed, location: location, participant: participant,
                    membership: membership, cloudCanWrite: canWrite,
                    accountLockAttemptID: lock.attemptID,
                    expectedSession: session
                )
        ...
        recordJoinReceipt { $0.lock = .active }
        try await attachAfterValidatingInvitationAccess(
            remote: remote + confirmedClaims, location: location, participant: participant,
    ...
    private func attachAfterValidatingInvitationAccess(
        remote: [HouseholdFact],
        location: CloudLocation,
        participant: String,
        membership: AccountFamilyMembership,
        cloudCanWrite: Bool,
        ...
        try requireInvitationAttachmentSession(
            expectedSession,
            location: location,
            participant: participant,
        ...
        try attach(
            ...
            accountLockAttemptID: accountLockAttemptID
        )
    }

    private func requireInvitationAttachmentSession(
        _ expectedSession: DeviceSession,
        location: CloudLocation,
        participant: String,
        membership: AccountFamilyMembership
    ) throws {
        guard session == expectedSession,
              expectedSession.selectedMemberID == nil,
              expectedSession.accountMembershipClaimBinding == nil else {
            throw HouseholdError.accountMembershipConflict
        }
        if let pending = expectedSession.pendingInvitationAcceptance {
            guard pending.location == location,
                  pending.cloudParticipantID == participant,
                  pending.invitationID == nil || pending.invitationID == membership.invitation.id else {
                throw HouseholdError.accountMembershipConflict
            }
        }
        if let householdID = expectedSession.householdID {
            guard householdID == location.householdID,
                  expectedSession.location == location,
                  expectedSession.cloudParticipantID == participant,
                  profiles.isEmpty else {
                throw HouseholdError.accountMembershipConflict
            }
        } else {
            guard expectedSession.location == nil,
                  expectedSession.cloudParticipantID == nil else {
                throw HouseholdError.accountMembershipConflict
            }
        }
    }
    ...
    private func reconcileInactiveAccountMembershipLock(_ lock: AccountMembershipLock) async throws {
        ...
        try validate(remote, householdID: location.householdID)
        let imported = HouseholdSnapshot(facts: remote)
        guard let current = try await membershipEvidence(
            in: imported,
            location: location,
            participant: participant
        ).binding,
    ...
    private func reconcileExpiredAccountMembershipLock(_ lock: AccountMembershipLock) async throws {
        ...
        do {
            try validate(remote, householdID: location.householdID)
            let imported = HouseholdSnapshot(facts: remote)
            if let binding = try await membershipEvidence(
                in: imported,
                location: location,
                participant: participant
            ).binding {
    ...
    private func membershipEvidence(
        in imported: HouseholdSnapshot,
        location: CloudLocation,
        participant: String
    ) async throws -> (binding: String?, membership: AccountFamilyMembership?) {
        guard imported.household?.id == location.householdID else { return (nil, nil) }
        if location.isOwner {
            return (AccountMembershipBinding.owner(householdID: location.householdID), nil)
        }
        if let membership = try imported.committedAccountMembership(participantID: participant) {
            ...
            return (AccountMembershipBinding.invitation(membership), membership)
        }
        if imported.grants.contains(where: { grant in
            ...
            return (
                AccountMembershipBinding.legacyShared(
                    householdID: location.householdID,
                    participantID: participant
                ),
                nil
    ...
    private func incompleteOwnerConnectionIsAuthoritative(
        ...
        return lock?.householdID == householdID
            && lock?.attemptID == attemptID
            && lock?.state == .provisional
            && lock?.claimBinding == nil
    }

    private func markFamilyAccessLost(expectedSession: DeviceSession) throws {
        guard session == expectedSession else { throw HouseholdError.pendingChanges }
        ...
        cloudAccessBlocked = true
        cloudIsReadOnly = true
        syncMessage = "Family access needs attention"
    }

    private func handleMissingFamilyAccess(
        _ error: Error,
        location: CloudLocation,
        participant: String,
        expectedSession: DeviceSession,
        expectedAccountGeneration: UInt64
    ) async throws -> MissingFamilyAccessOutcome {
        guard let cloudError = error as? CKError,
              [.unknownItem, .permissionFailure, .zoneNotFound, .userDeletedZone].contains(cloudError.code) else {
            return .notMissing
        }
        guard let transport,
              transport.accountGeneration == expectedAccountGeneration else {
        ...
        try markFamilyAccessLost(expectedSession: expectedSession)
        return .accessLost
    }
    ...
    private func promotePendingNativeAcceptanceIfCommitted(
        ...
        try requirePendingInvitationSession(pending)
        try confirmPendingInvitationAcceptance(location: location, participant: participant)
        recordJoinReceipt { $0.nativeAcceptance = .yes }
        return true
    }
    ...
    private func identifyPendingInvitation(in location: CloudLocation) async throws -> FamilyInvitation {
        ...
        try requirePendingInvitationSession(pending)
        if let invitationID = pending.invitationID, invitation.id != invitationID {
            throw HouseholdError.accountMembershipConflict
        }
        pending.invitationID = pending.invitationID ?? invitation.id
        pending.expiresAt = pending.expiresAt ?? invitation.expiresAt
        try persistPendingInvitation(pending)
        let validationTime = try await transport.invitationValidationTime(in: location, clientTime: clock())
    ...
    private func requirePendingInvitationSession(_ pending: PendingInvitationAcceptance) throws {
        guard session.pendingInvitationAcceptance == pending,
              session.selectedMemberID == nil,
              session.accountMembershipClaimBinding == nil else {
            throw HouseholdError.accountMembershipConflict
        }
        if let householdID = session.householdID {
            guard householdID == pending.location.householdID,
                  session.location == pending.location,
                  session.cloudParticipantID == pending.cloudParticipantID,
                  session.accountMembershipLockAttemptID == pending.accountLockAttemptID,
                  profiles.isEmpty else {
                throw HouseholdError.accountMembershipConflict
            }
        } else {
            guard session.location == nil,
                  session.cloudParticipantID == nil else {
                throw HouseholdError.accountMembershipConflict
            }
        }
    }
    ...
    func retryScheduledInvitationCleanup() async throws -> TimeInterval? {
        ...
        do {
            ...
            return Self.pendingInvitationCleanupRetryDelay
        } catch HouseholdError.invitationNotFound
            where session.pendingInvitationAcceptance?.phase == .awaitingRedemption {
            return Self.pendingInvitationCleanupRetryDelay
        }
    }
    ...
    private func performInvitationCleanup() async throws {
        ...
        if shouldInspectRemote, pending.accountLockAttemptID == nil {
            let attemptID = try await acquireAccountMembershipLock(
                householdID: pending.location.householdID
            ).attemptID
            try requirePendingInvitationSession(pending)
            pending.accountLockAttemptID = attemptID
            try persistPendingInvitation(pending)
        }
        let remote: [HouseholdFact]?
        ...
        if let remote {
            ...
            if let membership = evidence.membership {
                ...
                try await attachAfterValidatingInvitationAccess(
                    remote: remote, location: pending.location, participant: pending.cloudParticipantID,
                    membership: membership, cloudCanWrite: canWrite,
                    accountLockAttemptID: lock.attemptID,
                    expectedSession: session
                )
    ...
    func reconcileAccountMembershipLock() async throws {
        ...
        if session.pendingInvitationAcceptance == nil,
        ...
        do {
            ...
            switch try await handleMissingFamilyAccess(
                error,
                location: location,
                participant: participant,
                expectedSession: expectedSession,
        ...
        do {
            ...
            if let membership {
                ...
                try await attachAfterValidatingInvitationAccess(
                    remote: remote, location: location, participant: participant,
                    membership: membership, cloudCanWrite: canWrite,
                    accountLockAttemptID: active.attemptID,
                    expectedSession: session
                )
    ...
    private func reconcileAccountMembershipLock(imported: HouseholdSnapshot, location: CloudLocation,
        ...
        guard let transport else { throw HouseholdError.cloudUnavailable }
        let diagnostics = transport.familyTransitionDiagnostics
        diagnostics.record(stage: .ownerMembershipValidation, outcome: .started,
                           householdID: location.householdID)
        var shouldMarkMembershipReadOnlyOnConflict = false
        do {
            diagnostics.recordAccountIdentity(
                participantID: participant,
            ...
            diagnostics.recordExpectedOwnerMembership(
                ...
                memberID: selectedMember?.id,
                role: selectedMember?.role,
                locationIsOwner: location.isOwner,
                participantID: participant
            )
            ...
            if location.isOwner {
                ...
                diagnostics.record(stage: .lifecycleAuthorityValidation, outcome: .started,
                                   householdID: location.householdID)
                guard lifecycleState == .active else {
                    diagnostics.record(stage: .lifecycleAuthorityValidation, outcome: .failed,
                                       householdID: location.householdID,
                                       error: HouseholdError.accountMembershipConflict)
                    throw HouseholdError.accountMembershipConflict
                }
                diagnostics.record(stage: .lifecycleAuthorityValidation, outcome: .succeeded,
                                   householdID: location.householdID)
            }
            shouldMarkMembershipReadOnlyOnConflict = true
            guard transport.accountGeneration == expectedAccountGeneration else { throw HouseholdError.wrongAccount }
            diagnostics.record(stage: .membershipLockRead, outcome: .started,
                               householdID: location.householdID)
            let currentLock: AccountMembershipLock?
            do {
                currentLock = try await transport.accountMembershipLock()
                diagnostics.record(stage: .membershipLockRead,
                                   outcome: currentLock == nil ? .absent : .succeeded,
                                   lock: currentLock,
                                   householdID: location.householdID)
            } catch {
                diagnostics.record(stage: .membershipLockRead, outcome: .failed,
                                   householdID: location.householdID, error: error)
                throw error
            }
            guard transport.accountGeneration == expectedAccountGeneration else { throw HouseholdError.wrongAccount }
            let expectedOwnerBinding = AccountMembershipBinding.owner(householdID: location.householdID)
            let expectedOwnerAuthorityBinding = AccountMembershipBinding.ownerAuthority(participantID: participant)
            diagnostics.recordMembershipLock(
                currentLock,
                expectedOwnerBinding: expectedOwnerBinding,
                expectedOwnerAuthorityBinding: expectedOwnerAuthorityBinding,
            ...
            guard let binding = try await membershipEvidence(
                ...
                diagnostics.recordOwnerBranch(.noDerivedMembershipBindingValidateRetainedAttempt,
                                              result: "selected")
            ...
            if let localBinding = session.accountMembershipClaimBinding, localBinding != binding {
                diagnostics.recordOwnerBranch(.localClaimBindingConflict, result: "failed")
                throw HouseholdError.accountMembershipConflict
            }
            if let currentLock {
                if comparison.permitsActiveOwnerReuse {
                    diagnostics.recordOwnerBranch(.reuseExactActiveOwnerMembership, result: "selected")
                    let active = try await transport.activateAccountMembershipLock(
                        householdID: location.householdID,
                ...
                diagnostics.recordOwnerBranch(.validateRetainedAttemptThenAcquire, result: "selected")
                try validateRetainedMembershipAttempt(currentLock, location: location)
            } else {
                diagnostics.recordOwnerBranch(.acquireMissingMembershipLock, result: "selected")
            }
            ...
            diagnostics.recordOwnerBranch(.activateMembershipLock, result: "selected")
            let active = try await transport.activateAccountMembershipLock(
                householdID: location.householdID,
                attemptID: lock.attemptID,
                claimBinding: binding,
            ...
            diagnostics.record(stage: .ownerMembershipValidation, outcome: .failed,
                               householdID: location.householdID,
                               error: HouseholdError.accountMembershipConflict)
            if shouldMarkMembershipReadOnlyOnConflict {
                var updated = session
                updated.cloudCanWrite = false
                try repository.commit(facts: [], session: updated)
                session = updated
    ...
    private func ownerMembershipComparison(
        lock: AccountMembershipLock?,
        binding: String,
        imported: HouseholdSnapshot,
        location: CloudLocation,
        participant: String
    ) -> InvitationOwnerMembershipComparison {
        let parent = selectedMember
        let localBinding = session.accountMembershipClaimBinding
        return InvitationOwnerMembershipComparison(
            locationIsOwner: location.isOwner,
            sessionHouseholdMatchesLocation: session.householdID == location.householdID,
            sessionLocationMatches: session.location == location,
            sessionParticipantMatchesCurrentAccount: session.cloudParticipantID == participant,
            selectedMemberPresent: parent != nil,
            selectedMemberRoleIsParent: parent?.role == .parent,
            selectedMemberExistsAsParentInFetchedJournal: parent.map {
                imported.member($0.id)?.role == .parent
            } ?? false,
            derivedBindingMatchesExpectedOwner: binding == AccountMembershipBinding.owner(
                householdID: location.householdID
            ),
            lockHouseholdMatchesLocation: lock?.householdID == location.householdID,
            lockStateIsActive: lock?.state == .active,
            lockClaimBindingPresent: lock?.claimBinding != nil,
            lockClaimBindingMatchesDerivedBinding: lock?.claimBinding == binding,
            lockOwnerAuthorityBindingPresent: lock?.ownerAuthorityBinding != nil,
            lockOwnerAuthorityBindingMatchesCurrentAccount: lock?.ownerAuthorityBinding
                == AccountMembershipBinding.ownerAuthority(participantID: participant),
            localClaimBindingPresent: localBinding != nil,
            localClaimBindingMatchesDerivedBinding: localBinding.map { $0 == binding }
        )
    }
    ...
    private func resumeAccountMembership(participant: String, requestedLocation: CloudLocation?,
        ...
        try await attachAfterValidatingInvitationAccess(
            remote: existing.1, location: existing.0, participant: participant,
    ...
    private func performSynchronization() async throws {
        ...
        do {
            ...
            let missingAccessOutcome: MissingFamilyAccessOutcome
            if let participant = expectedSession.cloudParticipantID {
                missingAccessOutcome = try await handleMissingFamilyAccess(
                    error,
                    location: location,
                    participant: participant,
                    expectedSession: expectedSession,
                ...
                missingAccessOutcome = .notMissing
            }
            if missingAccessOutcome == .familyDeleted { return }
            if missingAccessOutcome == .accessLost {
